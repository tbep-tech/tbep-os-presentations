...
$$
\ln\left(Chl\right) = \beta_0 + \beta_1 t + \beta_2 Sal + \beta_3 \sin\left(2\pi t\right) + \beta_4 \cos\left(2\pi t\right)
$$
...
$Chl$: chlorophyll (or other response endpoint), $t$: time,  $Sal$: hydrologic inputs
...
toplo <- wrtdsmods %>%  
  ...
  group_by(yr, qrt) %>% 
  summarise(resid = mean(resid, na.rm = T), .groups = 'drop')
...
hydplo <- hydest %>% 
  ...
  select(yr, Hydro = hyd_dev, qrt)

seascmps <- full_join(hydplo, toplo, by = c('yr', 'qrt')) %>% 
  group_by(qrt) %>%
  nest() %>% 
  mutate(
