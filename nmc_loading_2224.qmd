...
mohydat2224 <- read.csv(here('data/segmonthlyh2o_2224_allsources_allsegs.csv')) |> 
  filter(!bayseg == 5) |>
  mutate(
    bay_segment = case_when(
      ...
      bayseg %in% c(55, 6, 7) ~ 'Remainder Lower Tampa Bay'
    )
  ) |> 
  select(
    year = YEAR,
  ...
  summarise(
    hy_load = sum(hy_load),
    .by = c(bay_segment, year, month)
  )
mohydat <- bind_rows(mohydat, mohydat2224) |> 
  mutate(
    bay_segment = factor(bay_segment, levels = levs)
  ) |> 
  mutate(
